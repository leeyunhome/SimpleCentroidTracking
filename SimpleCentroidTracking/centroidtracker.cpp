...
void CentroidTracker::register_Object(int cX, int cY)
{
	...
	this->disappeard.insert({object_ID, 0});
	this->nextObjectID += 1;
}
	...
	else
	{
		...
		if (objectCentroids.size() >= inputCentroids.size())
		{
			...
			for (auto row : unusedRows)
			{
				...
				this->disappeard[objectID] += 1;

				if (this->disappeard[objectID] > this->maxDisappeared)
				{
					...
					this->disappeard.erase(objectID);
				}
			}
		}
		else 
